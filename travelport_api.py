...
mycursor.execute("SELECT property_id,chain_code, brand_code, hotel_code, hotel_name, primary_image_url,phone_number, email_address, hotel_status FROM travelport_venue_details")

myresult = mycursor.fetchall()

data=pd.DataFrame(myresult,columns=['property_id','chain_code','brand_code','hotel_code','hotel_name','primary_image_url','phone_number','email_address','hotel_status'])
...
active_property=pd.merge(active_property,data_in_db, how='left' ,left_on=['CHAIN_CD','PPTY_ID'], right_on = ['brand_code','hotel_code'])
print(active_property.shape)

active_property=active_property[active_property['in_db']!=1]
print(active_property.shape)

mycursor=mydb.cursor()
mycursor.execute("SELECT id, state_code FROM cities where state_code <>'' ")
myresult = mycursor.fetchall()
...
mycursor=mydb.cursor()
mycursor.execute("SELECT id, code  FROM countries ")
myresult = mycursor.fetchall()
...
print(active_property.shape[0])

for prop in range(49500,active_property.shape[0],1): 
    try:
        ...
        try:
            ...
            categories=[]
            urls=[]
            if isinstance(images_all,dict)==True and images_all['@Category'] in ['1','2','23']:
                ...
                max_width=0
                max_index=0
                for i in range(max(len(images_all),10)):
                    if images_all[i]['@Category'] in ['1','2','23']:
                        c=images_all[i]['Description']['@Caption']
                        c=c.replace("'","%*+")

                        cat=images_all[i]['@Category']
                        height=images_all[i]['ImageFormat']['@Height']

                        width=images_all[i]['ImageFormat']['@Width']
                        
                        if (int(width)>max_width and images_all[i]['@Category'] in ['1','2']):
                            max_width=int(width)
                            max_index=int(i)

                        url=images_all[i]['ImageFormat']['URL']
                        url=url.replace('http://','https://')
                        url_last7=url[-7:]
                        

                        if (int(height)>=800 and int(width)>=1000) or url_last7=="XXL.jpg":
                            captions.append(c)
                            categories.append(cat)
                            urls.append(url)
                        
                if len(urls)==0:
                    c=images_all[max_index]['Description']['@Caption']
                    c=c.replace("'","%*+")
                    captions.append(c)
                    
                    categories.append(images_all[max_index]['@Category'])
                    url=images_all[max_index]['ImageFormat']['URL']
                    url=url.replace('http://','https://')
                    urls.append(url)
        ...
        if url!="":
            ...
            data3=pd.DataFrame(index=range(len(url_new)),columns=range(4))
            data3.columns=['name','description','src_url','travelport_id']
            ...
            images_new=(data3.groupby(['travelport_id'])
                .apply(lambda x: x[['name','description','src_url']].to_dict('records'))
                .reset_index()
        ...
        ws.cell(row,5).value=minutes
        ws.cell(row,6).value=response.status_code
        ws.cell(row,7).value=msg
        ws.cell(row,8).value=final_data
...
#print("Venues without images: " + str(without_image))
